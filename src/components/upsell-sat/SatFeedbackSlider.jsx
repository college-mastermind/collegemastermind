  ...
  return (
    ...
    >
      ...
      <div className=" mx-auto flex items-start xl:mr-32 gap-2 mt-8 md:mt-10">
        <button onClick={handlePrev} className="md:text-2xl bg-[#447EF7] text-white p-1 rounded-full">
          <GoChevronLeft />
        </button>
        <div className="mt-1 flex gap-2 md:gap-3 mx-1">
          {feedbacks.map((_, index) => (
            <span
              key={index}
              className={`h-2 w-2 mt-1.5 md:w-3 md:h-3 rounded-full  ${
                currentIndex === index ? "bg-[#243DBC]" : "bg-gray-300"
              }`}
            ></span>
          ))}
        </div>
        <button onClick={handleNext} className="md:text-2xl bg-[#447EF7] text-white p-1 rounded-full">
          <GoChevronRight />
        </button>
      </div>
    </div>
